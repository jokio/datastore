import { run, compose } from '../ddd/fn';
// // import { configure, get, save } from '../ddd/provider/datastore'
// // import { uniqueId } from '../index';


// // const credentials = { "type": "service_account", "project_id": "jok-joker", "private_key_id": "0160dacd6be040366407db7fa8e8946eca488a2f", "private_key": "-----BEGIN PRIVATE KEY-----\\nwxTZqOdH8e8/rOkwh9TpRSmG\n-----END PRIVATE KEY-----\n", "client_email": "jok-joker@appspot.gserviceaccount.com", "client_id": "101163287036168278175", "auth_uri": "https://accounts.google.com/o/oauth2/auth", "token_uri": "https://accounts.google.com/o/oauth2/token", "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs", "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/jok-joker%40appspot.gserviceaccount.com" };

// // const data = { id: 1, name: 'ezeki' };

// // async function run() {
// // 	try {
// // 		const db = await configure(credentials);
// // 		const kind = 'Customers';
// // 		const state = await get({ db, kind, id: '123' });
// // 		const newState = await save({ db, kind })(state);
// // 	}
// // 	catch (err) {
// // 		console.log(err);
// // 	}
// // }

// // run();


// const step1 = (props) => (state, context) => {
// 	state.a++;
// 	console.log('step1', context);
// 	return new Promise(resolve => setTimeout(() => resolve(state), 1000));
// }

// const step2 = (props) => (...args) => {
// 	// state.a++;
// 	// console.log('step2', args);

// 	// return args[0];
// 	return new Promise(resolve => resolve(args[0]));
// }

// const step3 = (props) => (state, context) => {
// 	console.log('step3', state, context);
// 	state.a++;

// 	return state;
// 	// return new Promise(resolve => resolve(state));
// }


// const runWithContext = ctx => (...funcs) => compose(ctx, funcs);

// // function compose(ctx, initialState, ...funcs) {

// // 	if (funcs.length === 0) {
// // 		return arg => arg
// // 	}

// // 	return funcs.reduce(
// // 		(a, b) => async (...args) => b(await a(...args), ctx),
// // 		arg => Promise.resolve(initialState) // initial value, need for context, also to work on one item
// // 	);
// // }

// async function run() {
// 	const context = 'context';
// 	const state = { a: 1 };

// 	const t = await compose(
// 		context,
// 		step3({}),
// 	)(state)

// 	console.log(t)
// 	// const state3 = await step2({}, context)(await step1({}, context)(state));
// }

// // const t = runWithContext({ x: 2 });
// // t(
// // 	step1({}),
// // 	step2({}),
// // 	step1({}),
// // 	step2({}),
// // 	step1({}),
// // 	step2({})
// // )({})
// run();



const step = (props) => (state, context) => ({ ...state, ...props });




async function run2(...funcs) {

	const t = await run(
		step({ a: 1 }),
		step({ b: 1 }),
		step({ c: 1 })
	);

	console.log(t);
}


run2();
